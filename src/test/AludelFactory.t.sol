...
contract AludelFactoryTest is Test {
    ...
    function test_WHEN_setting_a_different_fee_bps_and_recipient_THEN_its_passed_to_new_aludels() public {
        ...
        assertTrue(
            newAludel.spyWasCalled(
                abi.encodeWithSelector(
                    IAludel.initialize.selector, START_TIME, owner, address(69), 69, abi.encode(bytes(""))
                )
            )
        );
    ...
    function test_WHEN_launching_an_aludel_THEN_ownership_is_transferred_AND_the_vault_factory_AND_bonusTokens_are_registered(
    ) public {
        Spy spiedAludel = Spy(
            factory.launch(
                address(spyTemplate),
        ...
        assertTrue(spiedAludel.spyWasCalled(abi.encodeWithSelector(Ownable.transferOwnership.selector, address(69))));
        assertTrue(
            spiedAludel.spyWasCalled(abi.encodeWithSelector(IAludel.registerVaultFactory.selector, address(420)))
        );
        assertTrue(
            spiedAludel.spyWasCalled(
    ...
    function test_WHEN_launching_an_aludel_THEN_its_initialized_AND_bps_and_recipient_set_at_construction_time_are_used(
    ) public {
        Spy spiedAludel = Spy(
            factory.launch(
                address(spyTemplate),
    ...
    function test_WHEN_calling_permissioned_methods_with_a_non_owner_account_THEN_it_reverts() public {
        ...
        vm.expectRevert(bytes("Ownable: caller is not the owner"));
        factory.addProgram(address(preexistingAludel), address(listedTemplate), "name", "http://stake.me", 123);
        vm.expectRevert(bytes("Ownable: caller is not the owner"));
    ...
    function test_GIVEN_a_delisted_program_THEN_it_CAN_be_added_again() public {
        factory.delistProgram(address(aludel));
        factory.addProgram(address(aludel), address(listedTemplate), "namerino", "http://stake.me", 123);
    ...
    function test_WHEN_adding_a_program_manually_AND_using_template_zero_THEN_it_reverts() public {
        ...
        factory.addProgram(address(preexistingAludel), address(0), "name", "http://stake.me", 123);
    }
    ...
    function test_GIVEN_an_already_added_program_WHEN_adding_it_manually_THEN_it_reverts() public {
        ...
        factory.addProgram(address(aludel), address(listedTemplate), "name", "http://stake.me", 123);
    }

    function test_WHEN_adding_a_program_manually_THEN_the_instance_is_registered_AND_a_program_AND_metadata_can_be_set_AND_it_CANNOT_be_added_again(
    ) public {
        factory.addProgram(address(preexistingAludel), address(listedTemplate), "name", "http://stake.me", 123);
    ...
    function test_WHEN_launching_an_aludel_with_an_unlisted_template_THEN_it_reverts_with_TemplateNotRegistered()
        public
    {
        vm.expectRevert(AludelFactory.TemplateNotRegistered.selector);
        factory.launch(
    ...
    function test_WHEN_adding_a_template_as_disabled_THEN_its_listed_as_disabled_AND_no_programs_can_be_launched_with_it_AND_programs_can_be_added_with_it(
    ) public {
        factory.addTemplate(address(unlistedTemplate), "bloop", true);
        ...
        factory.addProgram(address(preexistingAludel), address(unlistedTemplate), "name", "http://stake.me", 123);
        assertEq(factory.programs(address(preexistingAludel)).name, "name");
    }
    ...
    function test_WHEN_disabling_a_template_THEN_its_listed_as_disabled() public {
        factory.addTemplate(address(unlistedTemplate), "bloop", false);
    ...
    function test_WHEN_adding_an_already_added_tempalte_THEN_it_reverts() public {
        ...
        factory.addTemplate(address(listedTemplate), "bloop", false);
    }
