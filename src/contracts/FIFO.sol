...
library FIFO {
    ...
    function pop(StakesQueue storage queue)
        public
        returns (StakeData memory stake)
    {
    ...
    /// @param queue queue internal state
    function length(StakesQueue storage queue)
        public
        view
        returns (uint256)
    {
    ...
    function at(StakesQueue storage queue, uint256 index)
        public
        view
    ...
    function update(StakesQueue storage queue, uint256 index, uint256 amount)
        public
    {
        queue.values[queue.first + index].amount = amount;
    }

    function values(StakesQueue storage queue)
        public
        view
        returns (StakeData[] memory)
    {
