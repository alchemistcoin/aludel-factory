...
contract AludelV2 is IAludel, Ownable, Initializable, Powered {
    ...
    function getFutureUnlockedRewards(uint256 timestamp) public view returns (uint256 unlockedRewards) {
        ...
        unlockedRewards = calculateUnlockedRewards(
            _aludel.rewardSchedules, remainingRewards, _aludel.rewardSharesOutstanding, timestamp
        );
        // explicit return
    ...
    function getCurrentVaultReward(address vault) external view returns (uint256 reward) {
        // calculate rewards
        return calculateRewardFromStakes(
    ...
    function getCurrentVaultStakeUnits(address vault) public view returns (uint256 stakeUnits) {
        // calculate stake units
        return getFutureVaultStakeUnits(vault, block.timestamp);
    }

    function getFutureVaultStakeUnits(address vault, uint256 timestamp) public view returns (uint256 stakeUnits) {
        // calculate stake units
    ...
    /// @param permission bytes The signed lock permission for the universal vault
    function unstakeAndClaim(address vault, uint256 amount, bytes calldata permission)
        external
        override
        onlyOnline
        hasStarted
    {
        ...
        RewardOutput memory out = calculateRewardFromStakes(
            vaultData.stakes, amount, unlockedRewards, _aludel.totalStakeUnits, block.timestamp, _aludel.rewardScaling
        );
        ...
        if (out.reward > 0) {
            ...
            uint256 sharesToBurn = _aludel.rewardSharesOutstanding.mul(out.reward).div(remainingRewards);
            ...
            if (_bonusTokenSet.length() > 0) {
                for (uint256 index = 0; index < _bonusTokenSet.length(); index++) {
                    ...
                    uint256 bonusAmount =
                        IERC20(bonusToken).balanceOf(_aludel.rewardPool).mul(out.reward).div(remainingRewards);
