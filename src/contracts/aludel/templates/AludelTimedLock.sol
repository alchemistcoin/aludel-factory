...
import { IAludelTimedLock } from "./IAludelTimedLock.sol";
...
contract AludelTimedLock is IAludelTimedLock, Powered, Ownable, Initializable {
    ...
    function calculateTotalStakeUnits(FIFO.StakesQueue storage stakes, uint256 timestamp)
        internal
        view
        returns (uint256 totalStakeUnits)
    {
        ...
        for (uint256 index; index < length; index++) {
            // reference stake
            FIFO.StakeData memory stakeData = FIFO.at(stakes, index);
            // calculate stake units
            uint256 stakeUnits =
    ...
    ) internal view returns (RewardOutput memory out) {
    ...
    function unstakeAndClaim(
        ...
        if (out.newStakesCount == 0) {
            ...
            while (vaultData.stakes.length() > out.newStakesCount) {
            ...
            if (out.lastStakeAmount > 0) {
                ...
                // vaultData.stakes.at(out.newStakesCount.sub(1)).amount = out.lastStakeAmount;
                vaultData.stakes.update(out.newStakesCount.sub(1), out.lastStakeAmount);
            }
        }
