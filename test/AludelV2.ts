...
import { BigNumber, BigNumberish, Contract, Event, Wallet } from 'ethers'
...
import { AludelFactory__factory, AludelV2__factory, Aludel__factory } from '../typechain-types'
...
describe('AludelV2', function () {
  ...
  async function launchProgram(
    ...
    args: any[]
      ...
      const creationEvent = receipt.events?.find((e: Event) => e.event == 'ProgramAdded')
  ...
  describe('admin functions', function () {
    ...
    describe.skip('isValidVault', function () {
      let vault: Contract
      beforeEach(async function () {
        vault = await createInstance('Crucible', vaultFactory, user)
      })
    ...
    describe.skip('registerVaultFactory', function () {
      describe('as user', function () {
        it('should fail', async function () {
    ...
    describe.skip('removeVaultFactory', function () {
      describe('as user', function () {
        beforeEach(async function () {
          await geyser.connect(admin).registerVaultFactory(vaultFactory.address)
        })
    ...
    describe('rescueTokensFromRewardPool', function () {
      ...
      describe('when offline', function () {
        ...
        it('should fail', async function () {
          await expect(
            geyser.connect(admin).rescueTokensFromRewardPool(otherToken.address, admin.address, mockTokenSupply),
          ).to.be.revertedWith('Powered_NotOnline')
        })
      })
      describe('when shutdown', function () {
        beforeEach(async function () {
  ...
  describe('user functions', function () {
    ...
    describe('to invalid vault', function () {
        it('should fail', async function () {
          ...
          await expect(
            stake(user, geyser, vault, stakingToken, stakeAmount)
          ).to.be.revertedWith('InvalidVault')
        })
      })
      describe('with amount of zero', function () {
        it('should fail', async function () {
          ...
          ).to.be.revertedWith('NoAmountStaked')
        })
      })
