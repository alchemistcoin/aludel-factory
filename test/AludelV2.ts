...
import { mocks } from '../typechain-types/src/contracts'
import { AludelFactory__factory, AludelV2__factory, Aludel__factory, Powered__factory } from '../typechain-types'
import { AbiCoder } from '@ethersproject/abi'
import { DAYS, signPermission } from './utils'
  ...
  describe('admin functions', function () {
        ...
        describe('at first funding', function () {
          ...
          it('should transfer tokens', async function () {
            await expect(geyser.connect(admin).fund(amplInitialSupply, YEAR))
              ...
              .withArgs(admin.address, rewardPool.address, subtractFundingFee(amplInitialSupply))
          })
        })
        describe('at second funding', function () {
          ...
          describe('with no rebase', function () {
            ...
            it('should update state correctly', async function () {
              ...
              expect(data.rewardSharesOutstanding).to.eq(subtractFundingFee(amplInitialSupply).mul(BASE_SHARES_PER_WEI))
              expect(data.rewardSchedules.length).to.eq(2)
              expect(data.rewardSchedules[0].duration).to.eq(YEAR)
              expect(data.rewardSchedules[0].start).to.eq((await getTimestamp()) - 1)
              expect(data.rewardSchedules[0].shares).to.eq(subtractFundingFee(amplInitialSupply).mul(BASE_SHARES_PER_WEI).div(2))
              expect(data.rewardSchedules[1].duration).to.eq(YEAR)
              expect(data.rewardSchedules[1].start).to.eq(await getTimestamp())
              expect(data.rewardSchedules[1].shares).to.eq(subtractFundingFee(amplInitialSupply).mul(BASE_SHARES_PER_WEI).div(2))
            })
            it('should emit event', async function () {
            ...
            it('should transfer tokens', async function () {
              await expect(geyser.connect(admin).fund(amplInitialSupply.div(2), YEAR))
                .to.emit(rewardToken, 'Transfer')
                .withArgs(admin.address, rewardPool.address, subtractFundingFee(amplInitialSupply).div(2))
            })
          })
        })
        describe('after unstake', function () {
          ...
          describe('with partial rewards exausted', function () {
            ...
            it('should update state correctly', async function () {
              ...
              expect(data.rewardSharesOutstanding).to.eq(subtractFundingFee(amplInitialSupply).mul(BASE_SHARES_PER_WEI).mul(3).div(4))
              expect(data.rewardSchedules.length).to.eq(2)
            ...
            it('should emit event', async function () {
              await expect(geyser.connect(admin).fund(amplInitialSupply.div(2), rewardScaling.time))
                .to.emit(geyser, 'AludelFunded')
                .withArgs(subtractFundingFee(amplInitialSupply).div(2), rewardScaling.time)
            })
            it('should transfer tokens', async function () {
              await expect(geyser.connect(admin).fund(amplInitialSupply.div(2), rewardScaling.time))
                .to.emit(rewardToken, 'Transfer')
          ...
          describe('with full rewards exausted', function () {
            ...
            it('should update state correctly', async function () {
              ...
              expect(data.rewardSharesOutstanding).to.eq(subtractFundingFee(amplInitialSupply).mul(BASE_SHARES_PER_WEI).div(2))
              expect(data.rewardSchedules.length).to.eq(2)
              expect(data.rewardSchedules[0].duration).to.eq(rewardScaling.time)
              expect(data.rewardSchedules[0].shares).to.eq(subtractFundingFee(amplInitialSupply).mul(BASE_SHARES_PER_WEI).div(2))
              expect(data.rewardSchedules[1].duration).to.eq(rewardScaling.time)
    ...
    describe('registerVaultFactory', function () {
      ...
      describe('when online', function () {
        ...
        it('should emit event', async function () {
          await expect(geyser.connect(admin).registerVaultFactory(secondFactory.address))
      ...
      describe('when offline', function () {
        ...
        it('should update state', async function () {
          ...
          expect(await geyser.getVaultFactoryAtIndex(0)).to.be.eq(vaultFactory.address)
          expect(await geyser.getVaultFactoryAtIndex(1)).to.be.eq(secondFactory.address)
        })
        it('should emit event', async function () {
          await expect(geyser.connect(admin).registerVaultFactory(secondFactory.address))
            .to.emit(geyser, 'VaultFactoryRegistered')
      ...
      describe('when online', function () {
        describe('on first call', function () {
          describe('with address zero', function () {
            it('should fail', async function () {
              await expect(
                geyser.connect(admin).registerBonusToken(ethers.constants.AddressZero)
              ).to.be.revertedWithCustomError(geyser, 'InvalidAddress')
            })
          })
          describe('with geyser address', function () {
            it('should fail', async function () {
              await expect(geyser.connect(admin).registerBonusToken(geyser.address)).to.be.revertedWithCustomError(
                geyser, 'InvalidAddress',
              )
            })
          })
          describe('with staking token', function () {
            it('should fail', async function () {
              await expect(geyser.connect(admin).registerBonusToken(stakingToken.address)).to.be.revertedWithCustomError(
                geyser, 'InvalidAddress',
              )
            })
          })
          describe('with reward token', function () {
            it('should fail', async function () {
              await expect(geyser.connect(admin).registerBonusToken(rewardToken.address)).to.be.revertedWithCustomError(
                geyser, 'InvalidAddress',
              )
            })
          })
          describe('with rewardPool address', function () {
            it('should fail', async function () {
              await expect(geyser.connect(admin).registerBonusToken(rewardPool.address)).to.be.revertedWithCustomError(
                geyser, 'InvalidAddress',
              )
            })
          })
        ...
        describe('on second call', function () {
          ...
          describe('with same token', function () {
            it('should fail', async function () {
              await expect(geyser.connect(admin).registerBonusToken(bonusToken.address)).to.be.revertedWithCustomError(
                geyser, 'InvalidAddress',
              )
            })
          })
  ...
  describe('user functions', function () {
    ...
    describe('stake', function () {
      ...
      describe('when offline', function () {
        it('should fail', async function () {
          await powerSwitch.connect(admin).powerOff()
          await expect(stake(user, geyser, vault, stakingToken, stakeAmount))          .to.be.revertedWithCustomError(
            powered,
            'Powered_NotOnline',
          )
        })
    ...
    describe('to invalid vault', function () {
      ...
      describe('when funded', function () {
        ...
        describe('when MAX_STAKES_PER_VAULT reached', function () {
          ...
          it('should fail', async function () {
            await expect(stake(user, geyser, vault, stakingToken, stakeAmount.div(quantity)))
            .to.be.revertedWithCustomError(
              geyser,
              'MaxStakesReached',
            )
    ...
    describe('unstake', function () {
      ...
      describe('with default config', function () {
        ...
        describe('with amount of zero', function () {
          it('should fail', async function () {
            await expect(unstakeAndClaim(user, geyser, vault, stakingToken, 0))
            .to.be.revertedWithCustomError(
              geyser, 'NoAmountUnstaked',
            )
          })
        })
        describe('with amount greater than stakes', function () {
          it('should fail', async function () {
            await expect(unstakeAndClaim(user, geyser, vault, stakingToken, stakeAmount.add(1)))
            .to.be.revertedWithCustomError(
              geyser, 'InsufficientVaultStake',
            )
          })
        })
      })
